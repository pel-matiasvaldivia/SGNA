...
export default function EquiposCalibracionPage() {
  ...
  const getStatusColor = (estado: string, proxCal: string | null) => {
    if (estado === "fuera_servicio") {
      return { text: "text-muted-foreground", bg: "bg-muted/80 border-border", label: "Fuera de Servicio" };
    }
    if (estado === "vencido") {
      ...
      <div className="relative bg-white rounded-2xl border border-border p-6 shadow-xl overflow-hidden flex flex-col items-center justify-center">
        ...
        <div className="relative w-48 h-28 overflow-hidden flex items-end justify-center">
          <svg width="190" height="120" viewBox="0 0 200 120" className="overflow-visible select-none">
            ...
            {/* Center Pin */}
            <circle cx="100" cy="110" r="7" fill="#18181b" stroke="#0F172A" strokeWidth="2.5" />
          ...
          <div className="absolute bottom-0 text-center">
            <span className="text-3xl font-extrabold text-foreground block">{percent}%</span>
            <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">Calibrados</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-8 max-w-[1400px] mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight text-foreground">Calibración e Instrumentos</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {/* Stats summary cards */}
          <div className="bg-white rounded-2xl border border-border p-6 shadow flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400">
              <Wrench className="w-6 h-6" />
            </div>
            <div>
              <span className="text-xs text-muted-foreground block uppercase font-bold tracking-wider">Equipos en Inventario</span>
              <span className="text-2xl font-bold block text-foreground mt-1">{totalEquipos}</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-6 shadow flex items-center gap-4">
            ...
            <div>
              <span className="text-xs text-muted-foreground block uppercase font-bold tracking-wider">Calibración Vigente</span>
              <span className="text-2xl font-bold block text-foreground mt-1">{calibrados}</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-6 shadow flex items-center gap-4">
            ...
            <div>
              <span className="text-xs text-muted-foreground block uppercase font-bold tracking-wider">Calibración Vencida</span>
              <span className="text-2xl font-bold block text-foreground mt-1">{vencidos}</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-border p-6 shadow flex items-center gap-4">
            ...
            <div>
              <span className="text-xs text-muted-foreground block uppercase font-bold tracking-wider">Fuera de Servicio</span>
              <span className="text-2xl font-bold block text-foreground mt-1">{fueraServicio}</span>
            </div>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-border rounded-3xl p-12 text-center max-w-xl mx-auto shadow-2xl">
          <Wrench className="w-16 h-16 text-muted-foreground mx-auto mb-4 animate-bounce" />
          <h3 className="text-lg font-bold text-foreground mb-2">No se han ingresado equipos de medición</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 space-y-4">
            <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest px-1">Inventario General</h3>
            <div className="bg-white/30 backdrop-blur-md border border-border rounded-3xl shadow-xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-white/5 text-muted-foreground font-bold border-b border-border uppercase tracking-wider text-[10px]">
                      ...
                      return (
                        <tr 
                          ...
                          className={`hover:bg-white/5 transition-colors cursor-pointer ${
                            isSelected ? "bg-secondary/5 text-foreground font-semibold" : ""
                          }`}
                        >
                          <td className="p-4 font-mono font-bold">
                            <span className="bg-muted px-2 py-0.5 rounded text-foreground text-[10px]">
                              {e.codigo}
                            </span>
                          </td>
                          <td className="p-4 font-bold text-foreground text-sm">{e.nombre}</td>
          ...
          {selectedEquipo && (
            <div className="lg:col-span-4 space-y-6">
              <div className="bg-white border border-border rounded-3xl p-6 shadow-2xl space-y-6">
                <div className="border-b border-border pb-4">
                  ...
                  <h2 className="text-lg font-bold text-foreground mt-2">{selectedEquipo.nombre}</h2>
                  <p className="text-xs text-muted-foreground mt-1">
                    Número de Serie: <span className="text-foreground font-mono">{selectedEquipo.numero_serie || "Sin registrar"}</span>
                  </p>
                </div>
                ...
                <div className="space-y-4 text-xs">
                  <div className="flex justify-between border-b border-border pb-2">
                    <span className="text-muted-foreground">Intervalo Calibración:</span>
                    <span className="text-foreground font-semibold">{selectedEquipo.frecuencia_calibracion_meses} meses</span>
                  </div>
                  <div className="flex justify-between border-b border-border pb-2">
                ...
                <div className="space-y-3 pt-4 border-t border-border">
                  ...
                  {(!selectedEquipo.calibraciones || selectedEquipo.calibraciones.length === 0) ? (
                    <p className="text-muted-foreground text-xs italic">Aún no se han logueado controles ni calibraciones.</p>
                  ) : (
                      ...
                      {selectedEquipo.calibraciones.map((cal) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="text-[11px] text-muted-foreground">Realizado por: <span className="text-foreground">{cal.realizado_por}</span></p>
                            {cal.patron_utilizado && (
                              <p className="text-[10px] text-muted-foreground">Patrón: <span className="text-muted-foreground italic">{cal.patron_utilizado}</span></p>
                            )}
                            {certDoc && (
                              <div className="mt-2 pt-2 border-t border-border flex items-center gap-1.5 text-[10px] text-secondary">
      ...
      {isEquipoModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-white border border-border rounded-3xl p-6 shadow-2xl space-y-6">
            ...
            <form onSubmit={handleCreateEquipo} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Código Interno *</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-muted/10 border border-border text-foreground placeholder-muted-foreground text-sm focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-all"
                  />
                </div>
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Frecuencia (Meses) *</label>
                  <input
                    type="number"
                    min="1"
                    placeholder="12"
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-muted/10 border border-border text-foreground text-sm focus:outline-none focus:border-secondary transition-all"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Nombre / Instrumento *</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Marca</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Modelo</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Nº de Serie</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Ubicación Física</label>
                  <input
                    type="text"
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-border">
                <button
                  type="button"
                  onClick={() => setIsEquipoModalOpen(false)}
                  className="px-4 py-2.5 rounded-xl border border-border text-xs font-semibold text-muted-foreground hover:bg-muted hover:text-foreground transition-all"
                >
                  Cancelar
                </button>
                <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="w-full max-w-md bg-white border border-border rounded-3xl p-6 shadow-2xl space-y-6">
            ...
            <form onSubmit={handleCalibrateEquipo} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Fecha Ensayo *</label>
                  <input
                    type="date"
                    required
                    value={newCalFecha}
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Resultado *</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Realizado por (Laboratorio/Técnico) *</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Patrón Utilizado de Referencia</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Certificado Oficial (DMS)</label>
                <select
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-2">Comentarios Adicionales</label>
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-muted/10 border border-border text-foreground placeholder-muted-foreground text-sm focus:outline-none focus:border-secondary focus:ring-1 focus:ring-secondary transition-all resize-none"
                />
              </div>
